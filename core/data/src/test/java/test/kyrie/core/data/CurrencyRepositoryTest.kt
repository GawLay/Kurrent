...
class CurrencyRepositoryTest {
    ...
    @Test
    fun `API data is fetched correctly and saved to database`() =
        runTest {
            val supportedCurrenciesDto =
                SupportedCurrenciesResponseDto(
                    supportedCurrenciesMap =
                        mapOf(
                            "EUR" to
                                test.kyrie.core.data.model.CurrencyDto(
                                    currencyCode = "EUR",
                                    currencyName = "Euro",
                                    countryName = "European Union",
                                    countryCode = "EU",
    ...
    @Test
    fun `cached data is returned when cache is valid`() =
        runTest {
            // Valid cache
    ...
    @Test
    fun `conversion is saved to database`() =
        runTest {
            val saveConversion =
                SaveConversionDomain(
                    fromAmount = "100.00",
                    fromCurrency = "EUR",
                    toAmount = "117.65",
                    toCurrency = "USD",
    ...
    @Test
    fun `saved conversion is retrieved from database`() =
        runTest {
            // Saved conversion exists
            val savedEntity =
                SaveConversionEntity(
                    fromCurrency = "GBP",
                    fromAmount = "200.00",
                    toAmount = "273.97",
                    toCurrency = "USD",
    ...
    @Test
    fun `returns cached data when API fails`() =
        runTest {
    ...
    @Test
    fun `local currencies flow returns data from database`() =
        runTest {
            every { currencyDao.observeAllCurrencies() } returns flowOf(testCurrencies)
