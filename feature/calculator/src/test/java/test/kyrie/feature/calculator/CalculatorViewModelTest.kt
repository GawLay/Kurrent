...
class CalculatorViewModelTest {
    ...
    @Test
    fun `currency conversion calculates correctly`() =
        runTest {
    ...
    @Test
    fun `saved conversion is updated after calculation`() =
        runTest {
            coEvery { observeLocalCurrenciesUseCase() } returns flowOf(testCurrencies)
            coEvery { conversionCurrencyUseCase.getSavedConversionCurrency() } returns null
    ...
    @Test
    fun `UI state shows error on conversion failure`() =
        runTest {
            val invalidCurrency =
                CurrencyDomain(
                    code = "XXX",
    ...
    @Test
    fun `UI state updates when currency is changed`() =
        runTest {
            coEvery { observeLocalCurrenciesUseCase() } returns flowOf(testCurrencies)
            coEvery { conversionCurrencyUseCase.getSavedConversionCurrency() } returns null
    ...
    @Test
    fun `loads saved conversion on initialization`() =
        runTest {
            val savedConversion =
